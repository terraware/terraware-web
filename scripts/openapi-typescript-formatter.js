  ...
  const types = await openapiTS('http://localhost:8080/v3/api-docs', {
    formatter(node) {
      ...
      if (
        ...
        node.description.includes('TYPESCRIPT-OVERRIDE-TYPE-WITH-ANY')
      ) {
        return 'any';
      }
    },
  });
  const path = __dirname + '/../src/api/types/generated-schema-seedbank.ts';
  fs.writeFileSync(path, types);
  console.log(
    'SUCCESSFULLY completed type overrides and wrote to generated-types.ts'
  );
})();
