describe('Help', () => {
  describe('Homepage', () => {
    ...
    it('navbar should have all elements', () => {
      cy.get('.MuiToolbar-root').children().should('have.length', 4);
    });

    it('navigate to help page', () => {
      cy.get('#help-button-link').should('have.attr', 'href', '/help').should('have.attr', 'target', '_blank');
      cy.request(Cypress.config().baseUrl + "/help").its('status').should('eq', 200);
    });
  });
  ...
  describe('Help page', () => {
    ...
    it('navbar should only have icon', () => {
      cy.get('.MuiToolbar-root').children().should('have.length', 2);
