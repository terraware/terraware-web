...
describe('validateRow (count mode)', () => {
  ...
  it('rejects a fractional count (seed counts must be whole numbers)', () => {
    expect(validateRow(seedsAccession(), 'Other', false, 1.5, 'Seeds')).toBe(strings.INVALID_VALUE);
    expect(validateRow(seedsAccession(), 'Out-planting', false, 10.25, 'Seeds')).toBe(strings.INVALID_VALUE);
  });
...
describe('validateRow (weight mode)', () => {
  ...
  it('requires subset data to withdraw by weight against a seed-count balance (cross-unit)', () => {
    // The remaining balance is a seed count with no subset ratio, so a weight withdrawal can't be
    // converted. The server rejects this, so validation must fail here for every purpose.
    expect(validateRow(seedsAccession(), 'Other', true, 5, 'Grams')).toBe(
      strings.WITHDRAWAL_COUNT_GREATER_THAN_ZERO_ERROR
    );
    expect(validateRow(seedsAccession(), 'Out-planting', true, 5, 'Grams')).toBe(
      strings.WITHDRAWAL_COUNT_GREATER_THAN_ZERO_ERROR
    );
  });

  it('accepts a weight withdrawal against a seed-count balance when subset data is present', () => {
    // 5g subset == 100 seeds, so 2g == 40 seeds, within the 100 remaining.
    const acc = seedsAccession({ subsetWeight: { quantity: 5, units: 'Grams' }, subsetCount: 100 });
    expect(validateRow(acc, 'Other', true, 2, 'Grams')).toBe('');
  });
});
