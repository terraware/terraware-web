...
const WithdrawSeedsForm = ({ open, onClose, accessions, user, onWithdrawn }: WithdrawSeedsFormProps): JSX.Element => {
  ...
  const rowsToWithdraw = useMemo(
    () =>
      accessions
        ...
        .filter(({ quantity }) => !!quantity?.value && quantity.value > 0),
    [accessions, draft.withdrawByAccession]
  );
  ...
  const canGoNextFromStep1 = useMemo(() => {
    ...
    return true;
  }, [draft.date, draft.destinationFacilityId, draft.testType, fieldErrors.date, isNursery, isViability]);
  ...
  const submitNursery = useCallback(async (): Promise<string[]> => {
    const failed: string[] = [];
    ...
    for (const { accession, quantity } of rowsToWithdraw) {
      try {
        ...
        failed.push(accession.accessionNumber);
        break;
      }
    }
  ...
  const submitWithdrawal = useCallback(async (): Promise<string[]> => {
    ...
    return results
      .map((result, index) => (result.status === 'rejected' ? rowsToWithdraw[index].accession.accessionNumber : null))
      .filter((value): value is string => value !== null);
  }, [createWithdrawal, draft, rowsToWithdraw]);

  const submitViabilityTest = useCallback(async (): Promise<string[]> => {
    ...
    try {
      ...
      return [accession.accessionNumber];
    }
  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      let failed: string[];
      if (isNursery) {
        failed = await submitNursery();
      } else if (isViability) {
        failed = await submitViabilityTest();
      } else {
        failed = await submitWithdrawal();
      }

      if (failed.length === 0) {
        trackEvent(MIXPANEL_EVENTS.ACCESSION_WITHDRAWN, {
          purpose: draft.purpose,
          quantity: rowsToWithdraw.length,
        });
        markSubmitted();
        ...
        // Prune succeeded rows so a retry does not double-withdraw them.
        const stillFailing = new Set(failed);
        setDraft((prev) => ({
          ...prev,
          withdrawByAccession: Object.fromEntries(
            Object.entries(prev.withdrawByAccession).filter(([accessionId]) => {
              const match = accessions.find((accession) => accession.id === Number(accessionId));
              return match ? stillFailing.has(match.accessionNumber) : false;
            })
          ),
        }));
        setStep((prev) => (prev === 2 ? 1 : prev) as FlowStep);
      }
    } finally {
      setIsCreating(false);
    }
  ...
  const handleWithdrawClick = useCallback(() => {
    // Warn if the withdrawal date predates any accession's received date.
    const withdrawalDate = draft.date ? new Date(draft.date) : undefined;
    const predatesReceived =
      withdrawalDate !== undefined &&
      accessions.some((accession) => accession.receivedDate && withdrawalDate < new Date(accession.receivedDate));
    if (predatesReceived) {
      setShowDateWarning(true);
      return;
    }
    void onSubmit();
  }, [accessions, draft.date, onSubmit]);
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Typography fontSize='14px' color={theme.palette.TwClrTxtSecondary}>
            {strings
              .formatString(accessions.length === 1 ? strings.X_ACCESSION : strings.X_ACCESSIONS, accessions.length)
              .toString()}
          </Typography>
