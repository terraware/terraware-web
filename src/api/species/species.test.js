...
test('getAllSpecies() handles axios errors', async () => {
  ...
  await expect(getAllSpecies()).resolves.toEqual({ species: [], requestSucceeded: false });
});
...
test('updateSpecies() handles axios errors', async () => {
  ...
  await expect(updateSpecies(species, 1)).resolves.toEqual({ requestSucceeded: false });
});
