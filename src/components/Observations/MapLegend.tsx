...
export default function MapLegend({title, items}: MapLegendProps): JSX.Element {
  ...
  return <Box display='flex' justifyItems='flex-start' flexWrap='wrap' sx={{
...
function LabeledSwatch({ borderColor, fillColor, label }: LabeledSwatchProps): JSX.Element {
  ...
  return <Box display='flex' alignItems='center' marginRight={theme.spacing(3)}>
    <Box sx={{
    ...
    <Typography fontSize='12px' fontWeight={400}>{label}</Typography>
  </Box>;
}
