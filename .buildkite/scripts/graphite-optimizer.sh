...
if [[ "${BUILDKITE_PULL_REQUEST:-false}" == "false" ]]; then
  echo "No PR context (BUILDKITE_PULL_REQUEST=false); skipping Graphite CI optimizer."
  exit 0
fi

echo "Asking Graphite CI optimizer about PR #${BUILDKITE_PULL_REQUEST}..."
...
request_json=$(jq -n \
  ...
  '{
    token: $token,
    caller: { name: "terraware-web-buildkite", version: "1" },
    context: {
      kind: "BUILDKITE",
...
if [[ "$should_skip" != "true" ]]; then
  echo "Graphite CI optimizer: running CI. ${reason:-(no reason given)}"
  exit 0
fi
